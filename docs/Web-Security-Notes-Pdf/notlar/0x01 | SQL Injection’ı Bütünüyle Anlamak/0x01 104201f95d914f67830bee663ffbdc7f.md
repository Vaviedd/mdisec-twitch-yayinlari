...
```
Neden WHERE table_schema = database()? Çünkü amaç: Sadece AKTİF veritabanının tablolarını çekmek. Sunucudaki diğer veritabanları listelemez.
...
Concat'i uname ve pass'i aynı satırda görmek için kullanıyoruz. Birbirine karışmasın diye araya :ilker:(ayırıcı, özel bir anlamı yok) konmuş.
...
Vavi'den notlar:PreparedStatement doğru kullanıldığında SQL Injection’a karşı çok sağlam bir savunmadır.
Sızmak için ya koddan bir hata yakalanmalı, ya da farklı bir zafiyet türü hedeflenmelidir.

String input = "Electronics' OR '1'='1";
